...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent =
      "#cc-overlay{position:fixed;inset:0;z-index:9999;background:rgba(43,38,32,0.55);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;font-family:'Poppins',sans-serif;}" +
      "#cc-card{width:100%;max-width:32rem;max-height:90vh;overflow-y:auto;background:#faf7f2;border:1px solid #ece4d6;border-radius:2rem;padding:40px;box-shadow:0 30px 80px -20px rgba(43,38,32,0.5);}" +
      "#cc-icon-wrap{display:flex;justify-content:center;margin-bottom:24px;}" +
      "#cc-icon{display:flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:16px;overflow:hidden;background:#fff;border:1px solid #ece4d6;box-shadow:0 8px 20px -8px rgba(43,38,32,0.3);}" +
      "#cc-icon img{width:100%;height:100%;object-fit:contain;}" +
      "#cc-title{font-family:'Cormorant Garamond',serif;font-weight:600;font-size:1.5rem;color:#2b2620;text-align:center;margin:0 0 12px;}" +
      "#cc-body{font-size:0.875rem;line-height:1.75;color:#6b6255;margin:0 0 32px;text-align:center;}" +
      ...
      ".cc-btn-primary{background:linear-gradient(135deg,#f0c988,#b48b56);color:#1a1408;padding:16px 0;box-shadow:0 16px 32px -14px rgba(140,109,52,0.6);}" +
      ".cc-btn-secondary{background:#fff;color:#2b2620;border:1.5px solid #ddd6c9;padding:12px 0;}" +
      ".cc-link{background:none;border:none;color:#8c6d34;font-family:'Poppins',sans-serif;font-size:0.75rem;text-align:center;margin-top:4px;cursor:pointer;padding:4px;text-decoration:underline;}" +
      ".cc-link:hover{color:#2b2620;}" +
      ...
      ".cc-row-label{font-weight:600;font-size:0.9rem;color:#2b2620;}" +
      ".cc-row-desc{font-size:0.78rem;color:#6b6255;margin-top:2px;}" +
  ...
  var ICON_HTML = '<img src="/assets/favicon/apple-touch-icon.png" alt="" width="48" height="48">';
  ...
  function buildOverlay() {
    ...
    overlay.innerHTML =
      '<div id="cc-card">' +
      '<div id="cc-icon-wrap"><div id="cc-icon">' + ICON_HTML + "</div></div>" +
